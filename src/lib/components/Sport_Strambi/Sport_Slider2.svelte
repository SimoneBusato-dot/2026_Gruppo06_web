<script>
    ...
    $effect(() => {
        const allMounted = videoSlides.filter(Boolean).length === videos.length;
        if (allMounted) el = [...videoSlides];
    });
    ...
    function updateVideos(newIndex) {
        const prev = videoElements[currentIndex];
        if (prev) { prev.pause(); prev.currentTime = 0; }
        currentIndex = newIndex;
        requestAnimationFrame(() => {
            videoElements[currentIndex]?.play().catch(() => {});
        });
    }
    ...
    onMount(() => {
        ...
        requestAnimationFrame(() => {
            if (videoSlides[0]) {
                computedSlideWidth = videoSlides[0].getBoundingClientRect().width;
            }
        ...
        if (videoSlides[0]) ro.observe(videoSlides[0]);
...
<div class="slider-outer">
    <div
        ...
        style="transform: translateX(-{currentIndex * (computedSlideWidth + GAP)}px);"
    >
        {#each videos as src, index}
            <div
                class="slider-slide"
                class:active={index === currentIndex}
                bind:this={videoSlides[index]}
            >
    ...
    <div class="navigation-container" bind:this={buttons}>
        ...
        <button class="nav-btn" onclick={nextSlide} aria-label="Next slide">
            <svg width="10" height="13" viewBox="0 0 10 13" fill="none">
                <path d="M9.70742 6.06082L1.25877 0.146764C0.728548 -0.224391 0 0.154931 0 0.802149V11.7291C9.81396 12.3763 0.728549 12.7556 1.25877 12.3845L9.70742 6.47043C9.8496 6.37091 9.8496 6.16034 9.70742 6.06082Z" fill="#F3F3F3"/>
            </svg>
        </button>
    </div>
</div>
